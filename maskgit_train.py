...
def train(config, monitor):
    ...
    train_loader, valid_loader = npy_to_loader(
        "/home/lijiahao/workbench/sc-maskgit/data/data_bins.npy",
        "/home/lijiahao/workbench/sc-maskgit/data/condition.npy",
        batch_size=64,
    )
    ...
    scheduler = torch.optim.lr_scheduler.SequentialLR(
        optimizer,
        milestones=[config["lr_milestone"]],
    ...
    for _ in range(int(epoch)):
        ...
        with torch.no_grad():
            for images, texts in valid_loader:
                ...
                texts = texts.cuda()
                loss = base_maskgit(images, texts=texts)
                monitor.log({"Valid BCE loss": loss.item()})

    torch.save(base_maskgit.state_dict(), "./ckpt/maskgit.pt")


if __name__ == "__main__":
    config = {
        "lr": 0.0005515075852793089,
        "lr_milestone": 200,
        "max_step": 800,
        "weight_decay": 0.0002466668384425943,
    }
